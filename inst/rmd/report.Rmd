    ...
    for(f in files) {
      ...
      for(n in names(temp)){
        for(i in 1:length(temp[[n]])){
          bootstrap.data <- rbind(bootstrap.data, c(n, temp[[n]][i]))
        }
      }
    }
    colnames(bootstrap.data) <- c("algorithm", "score")
  }
...
### Bootstrapped Bulks
```{r bootstrap_plots, fig.width = 16, fig.height = 9}
ggplot2::ggplot(bootstrap.data, aes(x = algorithm, y = score, col = algorithm)) + geom_boxplot()
```

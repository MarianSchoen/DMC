...
```{r real_results, echo=FALSE}
  require(ggplot2)
  files <- list.files(
    paste(temp.dir, "/results/real", sep = ""),
    full.names = TRUE,
    pattern = "deconv.*.h5"
  )
  results.lists <- list()
  if (length(files) > 0) {
    ...
    if (length(unique(real.df$algorithm)) > length(algorithm.names)) {
  	  algorithm.names <- unique(real.df$algorithm)
    }
    ...
    if (length(files) > 0) {
      ...
      bootstrap.plot.width <- 10
      bootstrap.plot.height <- 10
    }
...
if (exists("score.plot")) {
	plot(score.plot)
	pdf(
    paste(plot.dir, "score_plot_real.pdf", sep = "/"),
    width = score.width,
    height = score.height
  )
	plot(score.plot)
...
pdf(
  paste(plot.dir, "scatter_plots_real.pdf", sep = "/"),
  width = score.width,
  height = 6
)
for (l in scatter.plots.list) {
...
if (exists("runtime.plot")) {
	plot(runtime.plot)
	pdf(
    paste(plot.dir, "runtime_plot_real.pdf", sep = "/"),
    width = 16,
    height = 9
  )
	plot(runtime.plot)
...
if (exists("cond.num.plots")) {
  ...
  pdf(
    paste(plot.dir, "condition_number_plots_real.pdf", sep = "/"),
    width = 16,
    height = 9
  )
  plot(cond.num.plots$cond_num_plot)
...
if (exists("bootstrap.data")) {
  bootstrap.plot <- ggplot2::ggplot(
    bootstrap.data, aes(x = algorithm, y = score, col = algorithm)
  ) + facet_grid(rows = vars(cell_type)) +
  geom_boxplot() +
  ylim(0,1)
  plot(bootstrap.plot)
  pdf(
    paste(plot.dir, "bootstrap_plot_real.pdf", sep = "/"),
...
if (dir.exists(paste(temp.dir, "/results/simulation/bulks", sep = ""))) {
	results.lists <- list()
	files <- list.files(
    paste(temp.dir,"/results/simulation/bulks", sep=""),
    full.names = TRUE,
    pattern = "*.h5"
  )
	if (length(files) > 0) {
...
if (dir.exists(paste(temp.dir, "/results/simulation/samples", sep = ""))) {
	results.lists <- list()
	files <- list.files(
    paste(temp.dir, "/results/simulation/samples", sep = ""),
    full.names = TRUE,
    pattern = "*.h5"
  )
	if (length(files) > 0) {
...
if (dir.exists(paste(temp.dir, "/results/simulation/genes", sep = ""))) {
	results.lists <- list()
	files <- list.files(
    paste(temp.dir, "/results/simulation/genes/", sep=""),
    full.names = TRUE,
    pattern = "*.h5"
  )
	if (length(files) > 0) {
...
if (dir.exists(paste(temp.dir, "/results/simulation/subtypes", sep = ""))) {
	results.lists <- list()
	files <- list.files(
    paste(temp.dir, "/results/simulation/subtypes/", sep = ""),
    full.names = TRUE,
    pattern = "*.h5"
  )
	if (length(files) > 0) {
...
if (exists("score.plot.sim")) {
	plot(score.plot.sim)
  pdf(
    paste(plot.dir, "score_plot_simulated.pdf", sep = "/"),
    width = score.width,
    height = score.height
  )
  plot(score.plot.sim)
...
if (exists("scatter.plots.list.sim")) {
  ...
  pdf(
    paste(plot.dir, "scatter_plots_simulated.pdf", sep = "/"),
    width = score.width,
    height = 6
  )
	for (l in scatter.plots.list.sim) {
...
if (exists("sample.plots")) {
  ...
  pdf(
    paste(plot.dir, "sample_plots_simulated.pdf", sep = "/"),
    width = 16,
    height = 9
  )
  for (p in sample.plots$cell.type.plots) plot(p)
...
if (exists("gene.plots")) {
  ...
  pdf(
    paste(plot.dir, "gene_time_plot_simulated.pdf", sep = "/"),
    width = 16,
    height = 9
  )
  plot(gene.plots$runtime.plot)
...
if (exists("gene.plots")) {
	for (p in gene.plots$cell.type.plots)
		plot(p)
  pdf(
    paste(plot.dir, "gene_score_plots_simulated.pdf", sep = "/"),
    width = 16,
    height = 9
  )
...
if (exists("score.plot.subtype")) {
  ...
  pdf(paste(plot.dir, "subtype_plot.pdf", sep = "/"), width = 20, height = 10)
  plot(score.plot.subtype$overall)
  invisible(dev.off())
}else{
	cat("No data available for plotting.\n")

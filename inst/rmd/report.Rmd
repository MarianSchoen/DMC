...
```{r real_results, echo=FALSE}
  ...
  if(length(files) > 0){
    ...
    bootstrap.plot.height <- length(unique(bootstrap.data$algorithm)) * 3
    bootstrap.plot.width <- length(unique(bootstrap.data$cell_type)) * 2
  }
...
plot(score.plot)
pdf(paste(plot.dir, "score_plot_real.pdf", sep = "/"))
plot(score.plot)
dev.off()
```  
...
plot(runtime.plot)
pdf(paste(plot.dir, "runtime_plot_real.pdf", sep = "/"))
plot(runtime.plot)
dev.off()
```  
...
pdf(paste(plot.dir, "scatter_plots_real.pdf", sep = "/"))
for(l in scatter.plots.list){
	if(!is.null(l)){
		for(p in l){
			if(!is.null(p))
...
pdf(paste(plot.dir, "condition_number_plots_real.pdf", sep = "/"))
plot(cond.num.plots$cond_num_plot)  
...
if(exists("bootstrap.data")){
  bootstrap.plot <- ggplot2::ggplot(bootstrap.data, aes(x = algorithm, y = score, col = algorithm)) + facet_grid(rows = vars(cell_type)) + geom_boxplot()
  plot(bootstrap.plot)
  pdf(paste(plot.dir, "bootstrap_plot_real.pdf", sep = "/"))
  plot(bootstrap.plot)
  dev.off()
}
```
...
if(exists("score.plot.sim")){
	plot(score.plot.sim)
  pdf(paste(plot.dir, "score_plot_simulated.pdf", sep = "/"))
  plot(score.plot.sim)
  dev.off()
}
```  
...
if(exists("scatter.plots.list.sim")){
  ...
  pdf(paste(plot.dir, "scatter_plots_simulated.pdf", sep = "/"))
	for(l in scatter.plots.list.sim){
		for(p in l){
			plot(p)
		}
...
if(exists("sample.plots")){
  ...
  pdf(paste(plot.dir, "sample_plots_simulated.pdf", sep = "/"))
  for(p in sample.plots$cell.type.plots) plot(p)
  dev.off()
}
```  
...
if(exists("gene.plots")){
  ...
  pdf(paste(plot.dir, "gene_time_plot_simulated.pdf", sep = "/"))
  plot(score.plot.sim)
  dev.off()
}
```  
...
if(exists("gene.plots")){
	for(p in gene.plots$cell.type.plots)
		plot(p)
  pdf(paste(plot.dir, "gene_score_plots_simulated.pdf", sep = "/"))
	for(p in gene.plots$cell.type.plots)
		plot(p)
  dev.off()
}
...
if(exists("score.plot.subtype")){
  ...
  pdf(paste(plot.dir, "subtype_plot.pdf", sep = "/"))
  plot(score.plot.subtype)
  dev.off()
}
```  
...
if(exists("runtime.plot.subtype")){
  ...
  pdf(paste(plot.dir, "subtype_plot_runtime.pdf", sep = "/"))
  plot(runtime.plot.subtype)
  dev.off()
}
```  

#' this function adds rows filled with 0 for all missing cell types
#' to the estimates returned by an algorithm
#'
#' @param est.props matrix containing estimated proportions for each cell type
#' (cell type x bulk)
#' @param include.in.x vector containing the cell types that
#' should be present in est.props
...
complete_estimates <- function(est.props, include.in.x){
  ...
  for (ct in include.in.x) {
    if (!ct %in% rownames(est.props)) {
      ...
      est.props <- rbind(est.props, rep(0, ncol(est.props)))

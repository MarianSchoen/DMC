...
#' @param n.features.pre.pca 1 < integer , number of features
...
#'    "centroid" (= UPGMC).
#'
#' @return list of hclust() objects for all cell types
#'
#' @examples
get_hclust <- function(
  sc.counts,
  ...
  if (verbose) {
    print(paste0("Detected ", length(cell.types), " cell.types"))
    print(paste0("Starting to iteratively cluster them at ", Sys.time()))
  }
  ...
  for (cell.type in cell.types) {
    ...
    profile.names.of.this.type <- sc.pheno[[sample.name.column]][
    ...
    if (verbose) {
      print(
        paste0("There are ", length(intersected.profile.names), " profiles")
      )
    }
    ...
    tmp.profiles <- which(colnames(sc.counts) %in% profile.names.of.this.type)
    tmp.counts <- sc.counts[, tmp.profiles, drop = FALSE]

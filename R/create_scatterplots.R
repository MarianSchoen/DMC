#' create scatterplots of deconvolution results for all algorithms
# 'and cell types in the dataset
#'
#' @param results.list list of results as returned by deconvolute
#' @param real.props matrix containing real proportions for each cell type
#' '(cell type x bulk); optional, needed only if results.list does not contain
#' 'bulk.props'
...
#' specifying the plotting order
#' @return list containing one plot per algorithm, ordered by algorithm.order
...
create_scatterplots <- function(
  ...
  if (is.null(real.props)) {
    if (!"bulk.props" %in% names(results.list)) {
      stop("results.list must contain an entry bulk.props;
           alternatively supply real.props")
    }
  ...
  for (j in seq_len(length(results.list))) {
    ...
    for (res in results.list[[j]]) {
      ...
      # (containing real and estimates for each cell type)
      df <- data.frame()
      if (!length(cts) > 0) next
      for (i in seq_len(length(cts))) {
        t <- cts[i]
        temp.df <- data.frame(
          real = real.props[t, ],
          estimate = res$est.props[t, ]
        )
        ...
        df <- rbind(
          ...
          data.frame(
            ...
            type = t,
            repetition = j,
            cor = cor(real.props[t, ], res$est.props[t, ])
          )
        )
      }
  ...
  for (name in names(dfs)) {
    ...
    # store correlation for each algorithm and repetition in matrix
    cors <- matrix(0,
      nrow = length(unique(df$repetition)),
      ncol = length(unique(df$type))
    )
    ...
    for (r in unique(as.character(df$repetition))) {
      for (ct in unique(as.character(df$type))) {
        temp <- as.numeric(
          as.character(
            unique(df[which(df$repetition == r & df$type == ct), "cor"])
          )
        )
    ...
    # join type names with corresponding mean correlation
    #  across all repetitions for plot titles
    ...
    for (i in seq_len(length(labs))) {
      temp[i] <- paste(
        labs[i], "\nr = ",
        round(mean(cors[,labs[i]]), 2),
        " +/- ",
        round(sd(cors[,labs[i]]), 2),
        sep = ""
      )
    }
    names(temp) <- labs

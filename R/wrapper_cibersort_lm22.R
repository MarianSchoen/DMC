...
run_cibersort_lm22 <- function(
  ...
  # (a little hardcoded, but it has to suffice for now)
  ...
  rownames(est.props.fitted) <- c(
    "B", "cd4+", "nk", "mono", "cd8+", rownames(est.props), "malignant"
  )
  ...
  return(list(est.props = est.props, sig.matrix = ref.profiles, model = NULL))
}

...
run_music <- function(
  ...
  if(is.null(model)){
    ...
    if(any(table(pheno[, cell.type.column])[include.in.x] < 2)){
      # remove that cell type
      include.in.x <- include.in.x[-which(table(pheno[, cell.type.column])[include.in.x] < 2)]
    }
    basis <- MuSiC::music_basis(x = sc.exprs, clusters = cell.type.column,
    ...
    genes <- rownames(sigma)[which(!apply(sigma, 1, function(x){any(is.na(x))}))]
    sigma <- sigma[genes,]
    X <- X[genes,]
    Y <- Y[genes,]
  ...
  est.props <- apply(Y, 2, function(y){
    MuSiC::music.basic(Y = y, X = X, S = mS, Sigma = sigma, iter.max = iter.max, nu = nu, eps = eps)$p.weight
  })
  ...
  return(list(est.props = est.props, sig.matrix = NULL, model = model))
}

...
#'  as columns
#' @param bulk.props numeric matrix containing cell type proportions,
...
write_data <- function(
  ...
  if (!is.null(sc.counts) && !is.null(sc.pheno)) {
    if (!(is.matrix(sc.counts) || class(sc.counts) == "dgCMatrix") ||
        !is.data.frame(sc.pheno) || is.null(rownames(sc.counts)) ||
        is.null(colnames(sc.counts)) || is.null(rownames(sc.pheno)) ||
        is.null(colnames(sc.pheno))) {
      stop("Invalid counts and pheno data.")
    }
    rhdf5::h5createGroup(filename, "singlecell")
    rhdf5::h5write(
      as.vector(rownames(sc.counts)),
      filename,
      "singlecell/geneids",
      write.attributes = TRUE
    )
    rhdf5::h5write(
      as.vector(colnames(sc.counts)),
      filename,
      "singlecell/cellids",
      write.attributes = TRUE
    )
    ...
    for (name in colnames(sc.pheno)) {
      rhdf5::h5write(
        as.vector(sc.pheno[[name]]),
        filename,
        paste("singlecell/pheno", name, sep = "/")
      )
    }
  }
  # write bulks if present
  if (!is.null(bulk.counts)) {
    if (!(is.matrix(bulk.counts) || class(bulk.counts) == "dgCMatrix") ||
        is.null(rownames(bulk.counts)) || is.null(colnames(bulk.counts))) {
      stop(
    ...
    rhdf5::h5write(
      as.vector(rownames(bulk.counts)),
      filename,
      "bulk/geneids",
      write.attributes = TRUE
    )
    rhdf5::h5write(
      as.vector(colnames(bulk.counts)),
      filename,
      "bulk/sampleids",
      write.attributes = TRUE
    )
    rhdf5::h5write(
      as.matrix(bulk.counts),
      filename,
      "bulk/data",
      write.attributes = TRUE
    )

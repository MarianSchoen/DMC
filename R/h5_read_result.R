#' read results of deconvolution by different algorithms
#' as written by write_result_list. The function works recursively.
#'
#' @param filename string, which file should be read in?
#' @param content h5 file content;
#' used instead of filename in case of recursive function call
    ...
    if ("data" %in% names(content)) {
      ...
      if (!is.null(groupname)) {
        if (groupname == "est.props" || groupname == "bulk.props" ||
            groupname == "c.true" || groupname == "c.true.coarsly") {
